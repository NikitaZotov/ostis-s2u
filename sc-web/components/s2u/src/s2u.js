...
S2u.Editor.prototype = {


    init: function (params) {
        this.typesMap = {
            's2u-type-node': sc_type_node,
            's2u-type-node-const': sc_type_node | sc_type_const,
            's2u-type-node-const-group': sc_type_node | sc_type_const | sc_type_node_class,
            's2u-type-node-const-abstract': sc_type_node | sc_type_const | sc_type_node_abstract,
            's2u-type-node-const-material': sc_type_node | sc_type_const | sc_type_node_material,
            's2u-type-node-const-norole': sc_type_node | sc_type_const | sc_type_node_norole,
            's2u-type-node-const-role': sc_type_node | sc_type_const | sc_type_node_role,
            's2u-type-node-const-struct': sc_type_node | sc_type_const | sc_type_node_struct,
            's2u-type-node-const-tuple': sc_type_node | sc_type_const | sc_type_node_tuple,
            's2u-type-node-var': sc_type_node | sc_type_var,
            's2u-type-node-var-group': sc_type_node | sc_type_var | sc_type_node_class,
            's2u-type-node-var-abstract': sc_type_node | sc_type_var | sc_type_node_abstract,
            's2u-type-node-var-material': sc_type_node | sc_type_var | sc_type_node_material,
            's2u-type-node-var-norole': sc_type_node | sc_type_var | sc_type_node_norole,
            's2u-type-node-var-role': sc_type_node | sc_type_var | sc_type_node_role,
            's2u-type-node-var-struct': sc_type_node | sc_type_var | sc_type_node_struct,
            's2u-type-node-var-tuple': sc_type_node | sc_type_var | sc_type_node_tuple,
            's2u-type-edge-common': sc_type_edge_common,
            's2u-type-arc-common': sc_type_arc_common,
            's2u-type-arc-common-access': sc_type_arc_access,
            's2u-type-edge-const': sc_type_edge_common | sc_type_const,
            's2u-type-arc-const': sc_type_arc_common | sc_type_const,
            's2u-type-arc-const-perm-pos-access': sc_type_arc_access | sc_type_const | sc_type_arc_pos |
            sc_type_arc_perm,
            's2u-type-arc-const-perm-neg-access': sc_type_arc_access | sc_type_const | sc_type_arc_neg |
            sc_type_arc_perm,
            's2u-type-arc-const-perm-fuz-access': sc_type_arc_access | sc_type_const | sc_type_arc_fuz |
            sc_type_arc_perm,
            's2u-type-arc-const-temp-pos-access': sc_type_arc_access | sc_type_const | sc_type_arc_pos |
            sc_type_arc_temp,
            's2u-type-arc-const-temp-neg-access': sc_type_arc_access | sc_type_const | sc_type_arc_neg |
            sc_type_arc_temp,
            's2u-type-arc-const-temp-fuz-access': sc_type_arc_access | sc_type_const | sc_type_arc_fuz |
            sc_type_arc_temp,
            's2u-type-edge-var': sc_type_edge_common | sc_type_var,
            's2u-type-arc-var': sc_type_arc_common | sc_type_var,
            's2u-type-arc-var-perm-pos-access': sc_type_arc_access | sc_type_var | sc_type_arc_pos |
            sc_type_arc_perm,
            's2u-type-arc-var-perm-neg-access': sc_type_arc_access | sc_type_var | sc_type_arc_neg |
            sc_type_arc_perm,
            's2u-type-arc-var-perm-fuz-access': sc_type_arc_access | sc_type_var | sc_type_arc_fuz |
            sc_type_arc_perm,
            's2u-type-arc-var-temp-pos-access': sc_type_arc_access | sc_type_var | sc_type_arc_pos |
            sc_type_arc_temp,
            's2u-type-arc-var-temp-neg-access': sc_type_arc_access | sc_type_var | sc_type_arc_neg |
            sc_type_arc_temp,
            's2u-type-arc-var-temp-fuz-access': sc_type_arc_access | sc_type_var | sc_type_arc_fuz |
            sc_type_arc_temp
        };

        this.render = new S2u.Render();
        this.scene = new S2u.Scene({
            render: this.render,
            edit: this
        });
        this.scene.init();
    ...
    initUI: function () {
        ...
        $(tools_container).load('static/components/html/s2u-tools-panel.html', function () {
            $.ajax({
                url: "static/components/html/s2u-types-panel-nodes.html",
                dataType: 'html',
                success: function (response) {
                    self.node_types_panel_content = response;
                },
                error: function () {
                    S2uDebug.error("Error to get nodes type change panel");
                },
                complete: function () {
                    $.ajax({
                        url: "static/components/html/s2u-types-panel-edges.html",
                        dataType: 'html',
                        success: function (response) {
    ...
    tool: function (name) {
        return $('#' + this.containerId).find('#s2u-tool-' + name);
    },
    ...
    bindToolEvents: function () {
        ...
        select.click(function () {
            self.scene.setEditMode(S2uEditMode.S2uModeSelect);
        });
        select.dblclick(function () {
            self.scene.setModal(S2uModalMode.S2uModalType);
            self.onModalChanged();
            var tool = $(this);
            ...
            function stop_modal() {
                ...
                self.scene.setEditMode(S2uEditMode.S2uModeSelect);
                self.scene.setModal(S2uModalMode.S2uModalNone);
            }
            ...
            cont.find('.popover-title').append(
                '<button id="s2u-type-close" type="button" class="close">&times;</button>');
            $(container + ' #s2u-type-close').click(function () {
                stop_modal();
            });
            $(container + ' .popover .btn').click(function () {
                S2uTypeNodeNow = self.typesMap[$(this).attr('id')];
                stop_modal();
            });
        });
        this.toolEdge().click(function () {
            self.scene.setEditMode(S2uEditMode.S2uModeEdge);
        });
        this.toolEdge().dblclick(function () {
            ...
            $(container + ' .popover .btn').click(function () {
                S2uTypeEdgeNow = self.typesMap[$(this).attr('id')];
                stop_modal();
            });
        });
        this.toolBus().click(function () {
            self.scene.setEditMode(S2uEditMode.S2uModeBus);
        });
        this.toolContour().click(function () {
            self.scene.setEditMode(S2uEditMode.S2uModeContour);
        });
        this.toolLink().click(function () {
            self.scene.setEditMode(S2uEditMode.S2uModeLink);
        });
        ...
        this.toolChangeIdtf().click(function () {
            self.scene.setModal(S2uModalMode.S2uModalIdtf);
            $(this).popover({
                container: container
            });
            $(this).popover('show');
            ...
            var input = $(container + ' #s2u-change-idtf-input');
            // setup initial value
                ...
                if (e.keyCode == KeyCode.Enter || e.keyCode == KeyCode.Escape) {
                    ...
                    if (e.keyCode == KeyCode.Enter) {
                        ...
                        if (obj.text != input.val()) {
                            self.scene.commandManager.execute(new S2uCommandChangeIdtf(obj,
                                input.val()));
                        }
                    }
                    stop_modal();
            ...
            // process controls
            $(container + ' #s2u-change-idtf-apply').click(function () {
                var obj = self.scene.selected_objects[0];
                if (obj.text != input.val() && !self._idtf_item) {
                    self.scene.commandManager.execute(new S2uCommandChangeIdtf(obj, input.val()));
                }
                if (self._idtf_item) {
                    window.sctpClient.get_element_type(self._idtf_item.addr).done(function (t) {
                        self.scene.commandManager.execute(new S2uCommandGetNodeFromMemory(
                            obj,
                            t,
                            input.val(),
            ...
            $(container + ' #s2u-change-idtf-cancel').click(function () {
                stop_modal();
            });
        ...
        this.toolChangeType().click(function () {
            ...
            el = $(this);
            el.popover({
                content: (obj instanceof S2u.ModelEdge) ? self.edge_types_panel_content : self.node_types_panel_content,
                container: container,
                title: 'Change type',
                html: true,
                delay: {
            ...
            $(container + ' .popover .btn').click(function () {
                ...
                self.scene.selected_objects.forEach(function (obj) {
                    if (obj.sc_type != newType) {
                        command.push(new S2uCommandChangeType(obj, newType));
                    }
                });
                self.scene.commandManager.execute(new S2uWrapperCommand(command));
                self.scene.updateObjectsVisual();
                stop_modal();
            });
        });
        ...
        this.toolSetContent().click(function () {
            ...
            var input = $(container + ' #s2u-set-content-input');
            var input_content = $(container + " input#content[type='file']");
            var input_content_type = $(container + " #s2u-set-content-type");
                ...
                if (e.keyCode == KeyCode.Enter || e.keyCode == KeyCode.Escape) {
                    if (e.keyCode == KeyCode.Enter) {
                        ...
                        if (obj.content != input.val() || obj.contentType != input_content_type
                                .val()) {
                            self.scene.commandManager.execute(new S2uCommandChangeContent(obj,
                                input.val(),
            ...
            // process controls
            $(container + ' #s2u-set-content-apply').click(function () {
                var obj = self.scene.selected_objects[0];
                ...
                if (file != undefined) {
                    ...
                    fileReader.onload = function () {
                        ...
                        if (obj.fileReaderResult != scLinkHelper.fileArrayBuffer || obj.contentType !=
                            scLinkHelper.type) {
                            self.scene.commandManager.execute(new S2uCommandChangeContent(
                                obj,
            ...
            $(container + ' #s2u-set-content-cancel').click(function () {
                stop_modal();
            });
        });
        ...
        this.toolOpen().click(function () {
            ...
            var open_dialog = document.getElementById("s2u-tool-open-dialog");
            self.scene.clearSelection();
            ...
            S2uObjectBuilder.scene = self.scene;
    ...
    onSelectionChanged: function () {
        if (this.canEdit) {
            ...
            if (this.scene.selected_objects.length > 1) {
                ...
                if (this.scene.selected_objects[0] instanceof S2u.ModelNode) {
                    this.showTool(this.toolChangeIdtf());
                    this.showTool(this.toolChangeType());
                } else if (this.scene.selected_objects[0] instanceof S2u.ModelEdge) {
                    this.showTool(this.toolChangeType());
                } else if (this.scene.selected_objects[0] instanceof S2u.ModelContour) {
                    this.showTool(this.toolChangeIdtf());
                } else if (this.scene.selected_objects[0] instanceof S2u.ModelLink) {
    ...
    onModalChanged: function () {
        ...
        function update_tool(tool) {
            if (self.scene.modal != S2uModalMode.S2uModalNone)
    ...
    collectIdtfs: function (keyword) {
        ...
        if (selected_obj instanceof S2u.ModelNode) {
