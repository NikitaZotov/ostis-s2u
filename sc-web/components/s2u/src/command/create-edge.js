S2uCommandCreateEdge = function (source, target, scene) {
    this.source = source;
    this.target = target;
    this.scene = scene;
    this.edge = null;
...
S2uCommandCreateEdge.prototype = {

    constructor: S2uCommandCreateEdge,
    ...
    execute: function () {
        ...
        if (this.edge == null) {
            this.edge = S2u.Creator.createEdge(this.source, this.target, S2uTypeEdgeNow);
            scene.appendEdge(this.edge);
            var mouse_pos = new S2u.Vector2(scene.mouse_pos.x, scene.mouse_pos.y);
            var start_pos = new S2u.Vector2(scene.drag_line_points[0].x, scene.drag_line_points[0].y);
