S2uEdgeListener = function (scene) {
    this.scene = scene;
};

S2uEdgeListener.prototype = {

    constructor: S2uEdgeListener,
    ...
    onMouseDownObject: function (obj) {
        ...
        if (!scene.edge_data.source) {
            ...
            if (scene.edge_data.source != obj) {
                if (!(obj instanceof S2u.ModelContour && obj.isNodeInPolygon(scene.edge_data.source))) {
                    scene.commandManager.execute(new S2uCommandCreateEdge(scene.edge_data.source,
                        obj,
                        this.scene));
                    return true;
                } else {
